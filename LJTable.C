...
//    printf("Making this interaction tabulated. %i %i %i\n", i, j, K);
  ...
  else
  {
   ...
   int i_type = (soluteScalingOn && i >= table_dim_org)? ss_vdw_type[i-table_dim_org]:i;
   int j_type = (soluteScalingOn && j >= table_dim_org)? ss_vdw_type[j-table_dim_org]:j;
   ...
   if (soluteScalingOn && params->get_vdw_pair_params(i_type,j_type, &A, &B, &A14, &B14)) 
   {
